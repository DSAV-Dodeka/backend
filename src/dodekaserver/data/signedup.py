...
__all__ = ['get_signedup_by_id', 'get_signedup_by_email', 'upsert_su_row']
...
async def get_signedup_by_id(dsrc: Source, id_int: int) -> SignedUp:
    signedup_row = await retrieve_by_id(dsrc, SIGNEDUP_TABLE, id_int)
...
async def upsert_su_row(dsrc: Source, su_row: dict):
    try:
        result = await upsert_by_id(dsrc, SIGNEDUP_TABLE, su_row)
    except DbError as e:
