...
from apiserver.data.use import retrieve_by_id, get_largest_where, update_column_by_unique, insert
...
async def insert_key(dsrc: Source, conn: AsyncConnection, kid: str, iat: str, use: str) -> int:
    row = {KEY_ID: kid, KEY_ISSUED: iat, KEY_USE: use}
    return await insert(dsrc, conn, KEY_TABLE, row)


async def insert_jwk(dsrc: Source, conn: AsyncConnection, encrypted_jwk_set: str) -> int:
