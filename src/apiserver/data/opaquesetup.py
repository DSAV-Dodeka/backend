...
async def _get_opaque_row(dsrc: Source, id_int: int) -> Optional[dict]:
    key_row = await retrieve_by_id(dsrc, OPAQUE_SETUP_TABLE, id_int)

    return key_row


async def _get_opaque_setup(dsrc: Source) -> OpaqueSetup:
...
async def get_setup(dsrc: Source) -> str:
    return (await _get_opaque_setup(dsrc)).value
